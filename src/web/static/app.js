...
folderPicker.addEventListener("change", () => {
  ...
  pickedFolder.textContent = folderName
    ? `Dossier sélectionné : ${folderName}`
    : "Dossier sélectionné.";
...
runButton.addEventListener("click", async () => {
  ...
  if (!folderPath) {
    statusText.textContent = "Veuillez fournir un chemin de dossier valide.";
    statusText.style.color = "#a02626";
    return;
  }
  ...
  try {
    ...
    setLoading(false, "Analyse terminée.");
  } catch (error) {
    setLoading(false, `Erreur: ${error.message}`);
    statusText.style.color = "#a02626";
  }
});
...
function renderResults(data) {
  ...
  const confidenceMap = new Map(
    categorizations.map((item) => [item.filename, item])
  );
...
function updateSummary(categorizations, warnings, issues) {
  ...
  const duplicateIssue = issues.find(
    (issue) => issue.issue_type === "possible_duplicates"
  );

  summaryTotal.textContent = categorizations.length.toString();
  summaryLow.textContent = lowConfidence.length.toString();
  summaryWarnings.textContent = warnings.length.toString();
  summaryDuplicates.textContent = duplicateIssue
    ? duplicateIssue.affected_files.length.toString()
    : "0";
}
...
function renderAttention(issues) {
  ...
  if (!issues.length) {
    ...
    attentionList.innerHTML = "<li>Aucun point critique.</li>";
    return;
  }
...
function renderTree(folderStructure, confidenceMap) {
  ...
  if (!entries.length) {
    treeView.textContent = "Aucune hiérarchie disponible.";
    return;
  }
...
function renderWarnings(hierarchyWarnings, issues) {
  ...
  const issueSummaries = issues.map(
    (issue) => `${issue.issue_type} (${issue.severity})`
  );
...
function renderSuggestions(suggestions) {
  ...
  if (!suggestions.length) {
    ...
    suggestionsContainer.textContent = "Aucune suggestion.";
    return;
  }
